...
<%
    ...
    // 3. VO 방식
    VoMember dbData = (VoMember) request.getAttribute("voMemberOne");
%>
...
<main>
    ...
    <form class="pt-3 pb-5" name="frm_member_modify" action="/member/member_modified" method="post">
        <%--    <div class="mb-3 row"> row: 없으면 lave과 input이 2줄로 나오고 1줄 전체(왼쪽 처음부터 오른쪽 끝까지)를 사용하겠다는 의미, mb: Margin Bottom --%>
        <div class="row mb-3">
            <label for="staticMember" class="col-sm-1 ms-5 col-form-label">No.</label>
            <div class="col-sm-9">
                <input type="text" readonly class="form-control" id="staticMember" name="memberId"
                       value="<%=dbData.getMemberId()%>">
            </div>
        </div>
        <div class="row mb-3">
            <label for="staticUserId" class="col-sm-1 ms-5 col-form-label">Login ID</label>
            <div class="col-sm-9">
                ...
                <input type="text" readonly class="form-control" id="staticUserId" name="loginId"
                       value="<%=dbData.getLoginId()%>">
            </div>
        </div>
        <div class="row mb-3">
            <label for="staticName" class="col-sm-1 ms-5 col-form-label">성명</label>
            <div class="col-sm-9">
                <input type="text" class="form-control" id="staticName" name="name"
                       value="<%=dbData.getName()%>">
            </div>
        </div>
        <div class="row mb-3">
            <label for="inputRole" class="col-sm-1 ms-5 col-form-label">구분</label>
            <div class="col-sm-9">
                <input type="text" class="form-control" id="inputRole" name="role"
                       value="<%=dbData.getRole()%>">
            </div>
        </div>
